 ...
 * 배달의민족(사장님) 알림 파서.
 *
 * 패키지명은 앱 버전에 따라 다를 수 있어 본문 키워드("배달의민족"/"배민")로도 판별한다.
 * 실제 패키지명은 앱의 "알림 덤프" 화면에서 확인 후 [KNOWN_PACKAGES] 에 추가하면 정확도가 올라간다.
 */
class BaeminParser : OrderParser {
    ...
    override fun parse(data: NotificationData): Order? {
        ...
        return Order(
            ...
            customerRequest = ParseUtils.labeled(text, "요청사항", "요청 사항", "사장님께"),
    ...
    companion object {
        /** 확인되면 추가: 배민 사장님 앱 패키지명. */
        val KNOWN_PACKAGES = setOf("com.baemin.ceo", "com.baemin.presto")
        private val KEYWORDS = listOf("배달의민족", "배민")
